<template>
  ...
  <div class="wrapper w-100 mb-5">
    <v-text-field v-model="query" variant="solo" placeholder="Digite o nome do professor ou disciplina" class="mb-1"
      hide-details :prepend-inner-icon="isFetchingTeachers || isFetchingSubjects
          ? 'mdi-loading mdi-spin'
          : 'mdi-magnify'
        " clearable @input="onChangeQuery" @click:clear="clear" />
    <v-list v-if="processedResults.length && router.currentRoute.value.query.q" class="results" elevation="1">
      <v-list-item v-for="item in processedResults" :key="item.id" variant="plain" class="item" role="button"
        :name="item.name" @click="enterSearch(item.id, item.type, item.name)">
        <v-icon v-if="item.type" :icon="item.type === 'teacher' ? 'mdi-account' : 'mdi-book'" class="mr-3" />
        {{ item.name }}
      </v-list-item>
    </v-list>
  </div>
