...
const sigHistoryPreHandler: preHandlerAsyncHookHandler = async (
  request,
  reply,
) => {
...
export async function historyRoutes(app: FastifyInstance) {
  ...
  app.get(
    '/me',
    { preHandler: sigHistoryPreHandler },
    async (request, reply) => {
      const { ra } = request.query as { ra: number };
      const history = await HistoryModel.findOne(
        { ra },
        { curso: 1, grade: 1, ra: 1, _id: 0 },
      ).lean<{ curso: string; grade: string; ra: number }>();
      if (!history) {
