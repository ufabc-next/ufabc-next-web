...
const plugin: FastifyPluginAsyncZodOpenApi = async (app) => {
  ...
  app.post('/', { schema: sigHistorySchema }, async ({ sessionId, headers, body }, reply) => {
    ...
    const parsedHistory = await connector.getHistory(sessionId, viewState as string);
    ...
    const hasGraduationChanged = student.course !== dbStudent?.cursos[0].nome_curso;
...
const transformCategory = (category: 'free' | 'mandatory' | 'limited'): Categories => {
  if (category === 'free') {
